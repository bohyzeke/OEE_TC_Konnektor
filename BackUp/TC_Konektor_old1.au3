...
#AutoIt3Wrapper_Res_Fileversion=1.30.22
...
Func Subor()
   ...
   If $Filesize > 10 Then ; Ak je subor nieje prazdny alebo prilis maly
      ...
      If @error Then
		 Local $sString = StringReplace(_Now(), ":", "_")
		 $sString = StringReplace($sString, ".", "_")
		 Local $FBackup = $DirBack &"OEE" & $sString&".txt" ;Zalozny subor cestaskriptu / Backup/OEE_d_a_t_u_m.txt
...
;~       ConsoleWrite(@CRLF & $LogSize)
      If $LogSize >= "15000" Then
         ...
         FileMove($LogFile, $DataDir & $cas & "Archive.log", $FC_OVERWRITE & $FC_CREATEPATH)
